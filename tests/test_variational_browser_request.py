...
def test_selected_legs_maps_pair_direction():
    ...
    assert selected_legs("SHORT_BTC_LONG_ETH", "ETH", "close") == [("ETH", "SELL")]
...
def test_build_close_request_inverts_side_and_sets_reduce_only():
    requests = build_requests(
        args(direction="SHORT_BTC_LONG_ETH", action="close", legs="ETH", eth_quantity="0.0235"),
        {
            "BTC": fair("BTC", 100_000.0),
            "ETH": fair("ETH", 2_000.0),
        },
    ...
    assert len(requests) == 1
    order = requests[0]["variationalOrder"]
    assert order["symbol"] == "ETH"
    assert order["side"] == "SELL"
    assert order["quantity"] == "0.0235"

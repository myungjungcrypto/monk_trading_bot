...
class PacificaExchange(BaseExchange):
    """
    Pacifica Exchange API 커넥터.

    HMAC-SHA256 서명 기반 인증을 사용합니다.
    """

    name = "pacifica"
    BASE_URL = "https://api.pacifica.exchange"  # TBD
    WS_URL = "wss://ws.pacifica.exchange"  # TBD
    ...
    async def _request(
        ...
        try:
            ...
            logger.error("Pacifica API error: %s %s - %s", method, path, e)
            raise
    ...
    async def _handle_response(self, resp: aiohttp.ClientResponse) -> Any:
        ...
        raise PacificaAPIError(resp.status, error_msg)
    ...
    async def set_leverage(self, symbol: str, leverage: int) -> bool:
        try:
            ...
            logger.warning("Pacifica set_leverage failed: %s", e)
            return False
        ...
        async def _run():
            while self._ws_running:
                try:
                    ...
                    logger.info("Pacifica WS connected")

                    for sym in symbols:
                        await self._ws.send_json({"op": "subscribe", "channel": "ticker", "symbol": sym})
                        logger.info("Pacifica WS subscribed: %s", sym)
                    ...
                    async for msg in self._ws:
                        ...
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            try:
                                ...
                                logger.warning("Pacifica WS parse error: %s", e)
                        elif msg.type in (aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.ERROR):
                            break
                except Exception as e:
                    if self._ws_running:
                        logger.error("Pacifica WS error: %s, reconnecting in 3s...", e)
    ...
    async def disconnect_ws(self) -> None:
        ...
        if self._ws_task:
            ...
            self._ws_task = None
        logger.info("Pacifica WS disconnected")
    ...
    def perp_symbol(self, base: str) -> str:
        ...
        if sym is None:
            raise ValueError(f"Unknown base asset for Pacifica perp: {base}")
        return sym

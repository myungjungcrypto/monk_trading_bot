...
- 2026-07-03: Market order submit payload는 `{quote_id, side, max_slippage, is_reduce_only}` 형태다. 수량/심볼은 직전 `/api/quotes/simple` 또는 `/api/quotes/indicative` 응답의 quote id에 묶이는 구조로 보인다. Analyzer는 `Market Order Flows` 섹션에서 `orders/new/market` 요청을 matching quote response와 연결하고, auth token/JWT/signed_message 같은 민감한 JSON 필드는 redacted 처리한다.
- 2026-07-03: 실제 캡처에서 market order 4건이 모두 `POST /api/quotes/indicative` 응답의 `quote_id`를 받아 `POST /api/orders/new/market`에 넣는 흐름으로 매칭됐다. Quote 요청 payload는 `{instrument:{instrument_type:"perpetual_future", underlying:"BTC|ETH", funding_interval_s:3600, settlement_asset:"USDC"}, qty:"..."}`이고, submit payload는 `{quote_id, side:"buy|sell", max_slippage:0.005, is_reduce_only:true|false}`다. `tools/variational-browser`에 `npm start -- --api-shadow <request.json>`를 추가했다. 이 모드는 authenticated browser/CDP 세션에서 quote endpoint만 호출하고, 재구성한 market-order payload를 `tools/variational-browser/runtime/api-shadow/*.json`에 저장한다. Live submit endpoint `/api/orders/new/market`는 호출하지 않는다.

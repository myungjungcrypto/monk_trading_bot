...
- 완전 hands-off 복구를 하려면 `variational-wallet`이 상시 켜져 있어야 하고, 추가로 자동 승인 범위를 엄격히 제한해야 한다. 안전한 후보는 Variational 로그인용 `personal_sign`(도메인 `omni.variational.io`, URI `/api/auth/login`, 짧은 만료 시간)만 auto-approve하는 방식이다. 주문/송금/트랜잭션 계열 method까지 blanket auto-approve하면 실제 자금 이동까지 자동 승인될 수 있으므로 별도 명시 전에는 켜지 않는다.

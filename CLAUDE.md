...
- `VARIATIONAL_BROWSER_AUTO_CLICK_OPEN=true` 또는 자동 reduce-only 청산 경로에서는 Telegram preview/status screenshot 전송이 best-effort다. Telegram API가 일시적으로 `ETIMEDOUT`을 내더라도 request watcher와 자동 클릭 흐름은 계속 진행하고, 로그에 `[telegram] ... failed` 경고만 남긴다. 단, 수동 승인 모드는 승인 버튼을 받아야 하므로 Telegram 연결이 필요하다.
